...
Repozytorium Ligase jest publiczne, więc mechanizm aktualizacji z 2.4.26 działa "out of the box" — **bez tokena i bez żadnej konfiguracji** na stronie. Przycisk "Zaktualizuj teraz" pojawia się sam. (W 2.4.26 opis błędnie sugerował, że token jest wymagany.)

Token `LIGASE_GH_TOKEN` pozostaje wyłącznie jako opcja podnosząca limit zapytań API GitHuba (60 → 5000/h) — przydatne dopiero gdy wiele stron odpytuje z tego samego IP. Zwykłe instalacje go nie potrzebują.

Poza tym: poprawiony mylący komentarz w kodzie. Zero zmian funkcjonalnych względem 2.4.26.
...
Wbudowana biblioteka Plugin Update Checker (YahnisElsts, v5.7). Repozytorium jest publiczne, więc działa bez żadnej konfiguracji — token nie jest wymagany (patrz 2.4.27). Checker ładuje się tylko w panelu/cronie (zero narzutu na froncie) i instaluje ZIP z release'u (poprawna struktura katalogu `ligase/`).
